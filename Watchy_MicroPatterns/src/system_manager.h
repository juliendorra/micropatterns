...
#include <Watchy.h> // Includes Watchy RTC, WiFi, display specifics
#include <nvs.h>
#include <esp_sleep.h>
...
public:
    ...
    // RTC_Time getTime(); // Watchy::RTC.getRtcDateTime() returns a single struct
    // RTC_Date getDate(); // Watchy::RTC.getRtcDateTime() returns a single struct
    ::RTC_Date getDateTime(); // Returns Watchy's RTC_Date struct
    void printLocalTimeAndSetRTC(struct tm &timeinfo); // Helper for NTP

    // Power Management
    // Watchy has its own sleep handling, this might be simplified or integrated
    void goToSleep(TickType_t sleepDurationSec = 0); // Simplified, duration might be fixed by Watchy's RTC alarm
    esp_sleep_wakeup_cause_t getWakeupCause();
    void configureWakeupSources(bool forDeepSleep = false); // Watchy typically uses RTC + button for deep sleep wakeup
    void disableWakeupSources();   // Disabling GPIO wakeups after waking
