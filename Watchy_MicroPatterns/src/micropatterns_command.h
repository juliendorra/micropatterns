...
struct MicroPatternsState {
    uint16_t color = HAL_COLOR_BLACK; // Changed to HAL_COLOR_BLACK
    ...
    MicroPatternsState() : color(HAL_COLOR_BLACK), fillAsset(nullptr), scale(1.0f) { // Default color HAL_COLOR_BLACK
...
struct DisplayListItem {
    ...
    DisplayListItem() {
        // color is already defaulted by its declaration
        matrix_identity(matrix);
        matrix_identity(inverseMatrix);
    }
};
