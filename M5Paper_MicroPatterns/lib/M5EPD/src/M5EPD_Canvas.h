...
class M5EPD_Canvas : private TFT_eSprite {
   ...
   public:
    ...
    void qrcode(const char *string, uint16_t x = 50, uint16_t y = 10,
                uint16_t width = 220, uint8_t version = 6);
    ...
    void drawFastVLine(int32_t x, int32_t y, int32_t h, uint8_t thickness,
                       uint8_t color);
    void drawFastHLine(int32_t x, int32_t y, int32_t w, uint8_t thickness,
                       uint8_t color);
    ...
    bool drawJpgFile(fs::FS &fs, String path, uint16_t x = 0, uint16_t y = 0,
    ...
    bool drawJpgUrl(String url, uint16_t x = 0, uint16_t y = 0,
                    uint16_t maxWidth = 0, uint16_t maxHeight = 0,
                    uint16_t offX = 0, uint16_t offY = 0,
                    jpeg_div_t scale = JPEG_DIV_NONE);

    bool drawPngFile(fs::FS &fs, const char *path, uint16_t x = 0,
                     uint16_t y = 0, uint16_t maxWidth = 0,
                     uint16_t maxHeight = 0, uint16_t offX = 0,
    ...
    void pushImage(int32_t x, int32_t y, int32_t w, int32_t h,
                   const uint8_t *data);
    void pushImage(int32_t x, int32_t y, int32_t w, int32_t h, uint8_t transp,
                   const uint8_t *data);
    void fillCanvas(uint32_t color);
    void drawPixel(int32_t x, int32_t y, uint32_t color);
    // uint16_t alphaBlend(uint8_t alpha, uint16_t fgc, uint16_t bgc);
    uint16_t alphaBlend16(uint8_t alpha, uint8_t fgc, uint8_t bgc);
    void drawFreetypeBitmap(int32_t cx, int32_t cy, uint16_t bw, uint16_t bh,
                            uint8_t *bitmap);
    void clear(void);
...
// Parent functions - freetype
#ifdef FREETYPE_FONT
    void setTextFillMargin(uint16_t left, uint16_t right, int16_t top,
                           int16_t bottom) {
        TFT_eSPI::setTextFillMargin(left, right, top, bottom);
    }
    void setTextLineSpace(uint8_t space) {
        TFT_eSPI::setTextLineSpace(space);
    }
    esp_err_t loadFont(String path, fs::FS &ffs) {
        return TFT_eSPI::loadFont(path, ffs);
    }
    esp_err_t loadFont(const uint8_t *memory_ptr, uint32_t length) {
        return TFT_eSPI::loadFont(memory_ptr, length);
    }
    esp_err_t unloadFont() {
        return TFT_eSPI::unloadFont();
    }
    esp_err_t createRender(uint16_t size, uint16_t cache_size = 1) {
        return TFT_eSPI::createRender(size, cache_size);
    }
    esp_err_t destoryRender(uint16_t size) {
        return TFT_eSPI::destoryRender(size);
    }
    esp_err_t preRender(uint16_t code) {
        return TFT_eSPI::preRender(code);
    }
    bool isRenderExist(uint16_t size) {
        return TFT_eSPI::isRenderExist(size);
    }
    uint32_t getNumOfRender(void) {
        return TFT_eSPI::getNumOfRender();
    }
    void drawFreetypeGlyph(uint16_t code) {
        TFT_eSprite::drawFreetypeGlyph(code);
    }
    void useFreetypeFont(bool isuse = true) {
        TFT_eSPI::useFreetypeFont(isuse);
    }
#endif  // FREETYPE_FONT
    // Parent functions - other
    uint32_t getExceedOffset(void) {
        return TFT_eSPI::getExceedOffset();
    }
    void setTextArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
        TFT_eSprite::setTextArea(x, y, w, h);
    }
    size_t write(uint8_t utf8) {
        return TFT_eSprite::write(utf8);
    }
    ...
    int16_t textWidth(const char *string) {
        return TFT_eSPI::textWidth(string);
    }
    ...
    void drawCircleHelper(int32_t x0, int32_t y0, int32_t r, uint8_t cornername,
                          uint32_t color) {
        TFT_eSPI::drawCircleHelper(x0, y0, r, cornername, color);
    }
    void fillCircle(int32_t x0, int32_t y0, int32_t r, uint32_t color) {
        TFT_eSPI::fillCircle(x0, y0, r, color);
    }
    void fillCircleHelper(int32_t x0, int32_t y0, int32_t r, uint8_t cornername,
                          int32_t delta, uint32_t color) {
        TFT_eSPI::fillCircleHelper(x0, y0, r, cornername, delta, color);
    }
    void setWindow(int32_t x0, int32_t y0, int32_t x1, int32_t y1) {
        TFT_eSprite::setWindow(x0, y0, x1, y1);
    }
    void pushColor(uint32_t color) {
        TFT_eSprite::pushColor(color);
    }
    void pushColor(uint32_t color, uint16_t len) {
        TFT_eSprite::pushColor(color, len);
    }
    void writeColor(uint16_t color) {
        TFT_eSprite::writeColor(color);
    }
    void drawLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                  uint32_t color) {
    ...
    int16_t drawChar(uint16_t uniCode, int32_t x, int32_t y) {
        return TFT_eSprite::drawChar(uniCode, x, y);
    }
    int16_t drawChar(uint16_t uniCode, int32_t x, int32_t y, uint8_t font) {
        return TFT_eSprite::drawChar(uniCode, x, y, font);
    }
    void drawGlyph(uint16_t code) {
        TFT_eSprite::drawGlyph(code);
    }
    void printToSprite(String string) {
        TFT_eSprite::printToSprite(string);
    }
    void printToSprite(char *cbuffer, uint16_t len) {
        TFT_eSprite::printToSprite(cbuffer, len);
    }
    int16_t printToSprite(int16_t x, int16_t y, uint16_t index) {
        return TFT_eSprite::printToSprite(x, y, index);
    }
    ...
    void drawTriangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
        ...
        TFT_eSPI::drawTriangle(x0, y0, x1, y1, x2, y2, color);
    }
    void fillTriangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                      int32_t x2, int32_t y2, uint32_t color) {
        TFT_eSPI::fillTriangle(x0, y0, x1, y1, x2, y2, color);
    }
    ...
    void setTextDatum(uint8_t datum) {
        TFT_eSPI::setTextDatum(datum);
    }
    void setTextPadding(uint16_t x_width) {
        TFT_eSPI::setTextPadding(x_width);
    }
    int16_t drawNumber(long long_num, int32_t poX, int32_t poY, uint8_t font) {
        return TFT_eSPI::drawNumber(long_num, poX, poY, font);
    }
    int16_t drawNumber(long long_num, int32_t poX, int32_t poY) {
        return TFT_eSPI::drawNumber(long_num, poX, poY);
    }
    int16_t drawFloat(float floatNumber, uint8_t decimal, int32_t poX,
                      int32_t poY, uint8_t font) {
        return TFT_eSPI::drawFloat(floatNumber, decimal, poX, poY, font);
    }
    int16_t drawFloat(float floatNumber, uint8_t decimal, int32_t poX,
                      int32_t poY) {
        return TFT_eSPI::drawFloat(floatNumber, decimal, poX, poY);
    }
    int16_t drawString(const char *string, int32_t poX, int32_t poY,
                       uint8_t font) {
        return TFT_eSPI::drawString(string, poX, poY, font);
    }
    int16_t drawString(const char *string, int32_t poX, int32_t poY) {
        return TFT_eSPI::drawString(string, poX, poY);
    }
    int16_t drawCentreString(const char *string, int32_t dX, int32_t poY,
                             uint8_t font) {
        return TFT_eSPI::drawCentreString(string, dX, poY, font);
    }
    int16_t drawRightString(const char *string, int32_t dX, int32_t poY,
                            uint8_t font) {
        return TFT_eSPI::drawRightString(string, dX, poY, font);
    }
    ...
    uint16_t decodeUTF8(uint8_t *buf, uint16_t *index, uint16_t remaining) {
        return TFT_eSPI::decodeUTF8(buf, index, remaining);
    };
    uint16_t decodeUTF8(uint8_t *buf, uint16_t *index, uint16_t remaining,
                        uint8_t *length) {
        return TFT_eSPI::decodeUTF8(buf, index, remaining, length);
    };
    ...
    size_t print(struct tm *timeinfo, const char *format = NULL) {
        return Print::print(timeinfo, format);
    }
    ...
    size_t println(struct tm *timeinfo, const char *format = NULL) {
        return Print::println(timeinfo, format);
    }
    size_t println(void) {
