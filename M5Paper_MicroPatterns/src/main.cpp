...
void setup()
{
    ...
    if (!g_displayManager || !g_displayManager->initializeEPD())
    { // Initialize EPD early for messages
    ...
    if (!g_scriptManager || !g_scriptManager->initialize())
    { // Initializes SPIFFS
    ...
    if (!g_inputManager || !g_inputManager->initialize())
    { // Sets up GPIOs and ISRs
...
static bool triggerScriptRender(const String &humanIdToRender, bool useAsIsState, AppState &currentAppState_ref, String &currentLoadedScriptId_ref)
{
    if (humanIdToRender.isEmpty())
    {
        ...
        if (g_scriptManager->getScriptForExecution(defaultJobData.script_id, defaultJobData.file_id, defaultScriptState))
        { // No script_content
            if (!defaultJobData.script_id.isEmpty())
            {
                ...
                if (defaultScriptState.state_loaded)
                { // Default script state_loaded should be false from getScriptForExecution
    ...
    if (g_scriptManager->getScriptForExecution(jobData.script_id, fileId, scriptState))
    { // No scriptContentForJob
        ...
        if (!useAsIsState)
        { // Standard render: increment counter (if loaded), use current RTC time
...
void MainControlTask_Function(void *pvParameters)
{
    ...
    if (rtcTime.hour == 0 && rtcTime.min == 0 && rtcTime.sec == 0)
    { // RTC likely not set
    ...
    { // Same day, check time interval (e.g., 2 hours)
        int elapsed_minutes = (currentTime.hour - lf_hour) * 60 + (currentTime.min - lf_min);
        if (elapsed_minutes < 0)
            elapsed_minutes += 24 * 60; // Crossed midnight
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(g_inputEventQueue, &inputEvent, 0) == pdTRUE)
        {
            ...
            if (inputEvent.type == InputEventType::NEXT_SCRIPT || inputEvent.type == InputEventType::PREVIOUS_SCRIPT)
            {
                ...
                else
                {
                    ...
                    { // If selectNextScript provided an ID (even default)
        ...
        if (xQueueReceive(g_renderStatusQueue, &renderResultItem, 0) == pdTRUE)
        { // Use renderResultItem
            lastActivityTime = xTaskGetTickCount();
            ...
            {                                                                                               // Render failed (not success, not interrupted)
        ...
        if (xQueueReceive(g_fetchStatusQueue, &fetchResultItem, 0) == pdTRUE)
        { // Use fetchResultItem
            lastActivityTime = xTaskGetTickCount();
            ...
            else if (fetchResultItem.status == FetchResultStatus::SUCCESS)
            {
                ...
                if (fetchResultItem.new_scripts_available)
                {
                    ...
                    if (!currentStillValid && !currentLoadedScriptId.isEmpty())
                    {
                        ...
                        {
                            if (newJobData.script_id.isEmpty())
                            { // script_id is humanId
                            ...
                            else
                            {
                                ...
                                if (state.state_loaded)
                                    newJobData.initial_state.counter++;
                                else
                                    newJobData.initial_state.counter = 0;
                                RTC_Time now_time = g_systemManager->getTime();
            ...
            {                                                                                        // Other non-success, non-NO_WIFI, non-INTERRUPTED statuses (e.g., GENUINE_ERROR)
        ...
        if (currentState == AppState::IDLE && (xTaskGetTickCount() - lastActivityTime) > pdMS_TO_TICKS(SLEEP_IDLE_THRESHOLD_MS))
        {
            ...
            // This is now handled by InputManager, which will process the specific GPIO event
            // that caused the wakeup and draw the correct indicator type.
            // if (g_systemManager->getWakeupCause() == ESP_SLEEP_WAKEUP_GPIO) {
            //     g_displayManager->drawActivityIndicator(); // REMOVED
            // }
...
void RenderTask_Function(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(g_renderCommandQueue, &jobItem, queueReceiveTimeout) == pdTRUE)
        {
            ...
            if (g_displayManager->lockEPD(pdMS_TO_TICKS(1000)))
            { // Lock EPD, 1s timeout
...
void FetchTask_Function(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(g_fetchCommandQueue, &job, queueReceiveTimeout) == pdTRUE)
        {
            ...
            // Connect WiFi
            if (!g_networkManager->connectWiFi())
            { // connectWiFi uses its internal interrupt flag
                ...
                if (resultData.status == FetchResultStatus::SUCCESS)
                {
                    ...
                    else
                    {
                        ...
                        if (!g_scriptManager->saveScriptList(serverListDoc))
                        {
                            ...
                            if (serverListDoc.is<JsonArray>())
                            { // Check again, in case it was valid but save failed for other reasons
                        ...
                        else
                        {
                            ...
                            for (JsonObject scriptInfo : serverList)
                            {
                                ...
                                { // Other errors (GENUINE_ERROR, NO_WIFI)
