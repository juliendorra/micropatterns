...
{ "cmd": "provision",
  ...
  "tz": "CET-1CEST,M3.5.0,M10.5.0/3", // POSIX rule: offset + the switch dates
  "tzName": "Europe/Paris" }          // cosmetic, so `status` reads for humans
{ "cmd": "status" }                   // -> current SSID, RSSI, userId, tz, version
{ "cmd": "forget" }                   // wipe stored credentials; KEEPS the tz
```

All three parts of a `provision` — `userId`, `networks`, `tz` — are independent
and individually optional, with at least one required. That is what makes "I
moved country" a one-field write that does not ask for WiFi passwords again, the
same way ID rotation already avoided it.

`forget` deliberately keeps the timezone. Its button says *Erase WiFi + ID*, a
zone is not a credential, and nobody pressing that means "and put my watch back
on UTC".

The wire version is **2**. A v1 device silently ignores `tz`, so the editor reads
`version` from `status` first and says so rather than letting a write that did
nothing look like it landed.
...
ever be applied *at a sync*. Sending a single offset would mean a watch set up in
summer reads an hour late all winter, which is exactly the bug both firmwares
already had (each passed `daylightOffset_sec = 0` to `configTime`, because that
call has nowhere to say *when* a correction applies).

So the device is sent the zone's rules and hands them to newlib, which already
knows how to apply them: `configTzTime(tz, server)`. Two consequences:

- `mp_sync_scripts()` resyncs the clock on **every** script sync, while WiFi is
  already up. One SNTP round trip is a rounding error against fetching every
  script, and it means a device corrects itself within one sync of a switch. No
  "is it due yet" bookkeeping, which would need the date the Watchy cannot read.
- The strings are **looked up, not computed**. Every zone's POSIX string is
  published by IANA in the footer of its compiled TZif file; `posix_tz.js` is
  those footers, generated by `tools/gen_posix_tz.py`. Deriving them instead by
  probing browser `Intl` offsets across a year was tried and is a trap — it
  agreed with a reference libc on 414 of 418 zones and was confidently wrong on
  the rest, inventing a DST rule for `America/Vancouver` (which tzdata 2026c
  publishes as permanent `MST7`) and a two-rule approximation for
  `Africa/Casablanca` (for which tzdata declines to express a rule at all). See
...
user.id         str
tz.posix        str     POSIX TZ rule, <= 63 chars; empty = never set
tz.name         str     IANA name, cosmetic -- nothing on the device reads it
```

`tz.posix` is length-checked *before* the write, not after: NVS would truncate an
over-long string into one that still parses — `CET-1CE` is a valid standard-only
zone — so the device would run on a plausible wrong offset with nothing to show
it had happened.
