...
2.  **Integer Math:** All coordinates, parameters, and calculations use integers. Division truncates towards zero. A script cannot produce, store or observe a fractional value.
    ...
    *   **The renderer is integer from the display list to the pixels**, since 2026-09-04: rotation is a 360-entry table of exact integers, the cumulative angle and offset are exact integers, every endpoint, centre, bounding box and span boundary is computed from them, and the fill loops are `int32` add, shift and byte-wise mask writes. No `sinf`, no `sqrtf`, no float division, nothing per pixel or per row in floating point. One deliberate approximation is spent for that: the inverse transform treats `C² + S²` as exactly `2³⁰` (it is, to within 6e-5), which moves a handful of pattern-edge pixels per render and does not compound. The float renderer of 2026-09-02 stays selectable as `displaylist-float`, gated against its own goldens. See `docs/measurements/2026-09-04-integer-dda.md`.
