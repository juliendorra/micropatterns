...
4.  **Case Insensitive:** Keywords (e.g., `COLOR`, `PIXEL`) and parameter names (e.g., `X=`, `NAME=`) are case insensitive. String literals (e.g., `"my_icon"`) and variable names are case-sensitive.
...
*   **Parameters:** `NAME=value` format. Order generally doesn't matter unless specified. Values can be integer literals, string literals (in double quotes), variable references (`$variable_name`), or environment variables (`$HOUR`, etc.).
...
*   **Declaration:**
    ...
    VAR variable_name
    ```
    *   Declares a variable. Initial value is 0. Variable names are case-sensitive. Cannot conflict with environment variable names.

*   **Assignment:**
    ```micropatterns
    LET variable_name = expression
    ```
    *   Assigns the result of an expression to a *previously declared* variable.
    *   **Expressions:** Limited to integer math: `value [+-*/%] value ...`
        *   `value` can be an integer literal, `$variable`, or an environment variable (`$HOUR`, etc.).
    ...
    *   Example: `LET angle = $MINUTE * 6`
    *   Example: `LET offset = $COUNTER % 20 + 10`
...
*   **Patterns:**
    ...
    *   `NAME`: Case-sensitive string identifier (in double quotes).
...
*   **Icons:**
    ...
    *   Same parameters and constraints as `DEFINE PATTERN`.
...
*   **Pattern:**
    ...
    *   `NAME="pattern_name"`: Uses a previously defined pattern (name is case-sensitive string literal).
...
*   Parameters (`DX`, `DY`, `DEGREES`, `FACTOR`) can be integer literals or variables (`$var`).
